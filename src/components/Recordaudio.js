...
function Recordaudio(props) {
  ...
  const handleStart = async () => {
    ...
    recorder.addEventListener("stop", () => {
      setAudioBlob(new Blob([chunks], { type: "audio/x-wav" }));
      props.getblob(new Blob([chunks], { type: "audio/x-wav" }));
  ...
  return (
    ...
    >
      {!recording && <button onClick={handleStart}>Star t Recording</button>}
